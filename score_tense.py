...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Score the response of a system at tense classification.')
    ...
    for fname in os.listdir(args.key):
        ...
        if args.measurement == 'spans':
            ...
            path = os.path.join(args.key, fname)
            with open(path) as f: 
                key_event, key_tense = read_tokens_conll((2, 5), first_n_sentences(f, args.n), path)
            path = os.path.join(args.response, fname)
            if os.path.exists(path):
                with open(path) as f:
                    res_event, res_tense = read_tokens_conll((2, 5), first_n_sentences(f, args.n), path)
            else:
                res_event = res_tense = set() 
